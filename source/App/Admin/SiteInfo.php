...
class SiteInfo extends Admin
{
    ...
    public function home(?array $data): void
    {
        ...
        if (!empty($data["s"])) {
            $s = str_search($data["s"]);
            echo json_encode(["redirect" => url("/admin/config/home/{$s}/1")]);
            return;
        }
        ...
        if (!empty($data["search"]) && str_search($data["search"]) != "all") {
            ...
            if (!$posts->count()) {
                ...
                redirect("/admin/config/home");
            }
        }

        $all = ($search ?? "all");
        $pager = new Pager(url("/admin/config/home/{$all}/"));
    ...
    public function post(?array $data): void
    {
        ...
        if (!empty($data["action"]) && $data["action"] == "create") {
            ...
            $json["redirect"] = url("/admin/config/post/{$postCreate->id}");
        ...
        if (!empty($data["action"]) && $data["action"] == "update") {
            ...
            if (!$postEdit) {
                ...
                echo json_encode(["redirect" => url("/admin/config/home")]);
                return;
            }
